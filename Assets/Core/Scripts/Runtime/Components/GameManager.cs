...
namespace Blocks.Gameplay.Core
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        [Tooltip("Cursor lock mode applied after the local player enters gameplay.")]
        [SerializeField] private CursorLockMode gameplayCursorLockMode = CursorLockMode.Locked;
        [Tooltip("Whether the cursor is visible during gameplay.")]
        ...
        private void ClientConnected(ulong clientId)
        {
            ...
            // Setup player spawn position. CoreStatsHandler already initializes
            // fresh runtime stats from each definition's startingValue during
            // NetworkObject spawn, so applying another heal here would double
            // the player's initial health when the configured maximum exceeds 100.
            if (NetworkManager.Singleton != null && NetworkManager.Singleton.LocalClient != null)
            {
        ...
        private void OnSessionAdded(ISession session)
        {
            ...
            // CoreStatsHandler initializes a newly spawned player's health from
            // StatDefinition.startingValue. Do not add a second fixed heal here.
        }
