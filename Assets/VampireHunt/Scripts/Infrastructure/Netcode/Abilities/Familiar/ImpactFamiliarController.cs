...
{
    /// <summary>
    /// 撞击水滴使魔（impact familiar）的控制器：管理<b>多只</b>使魔共存，驱动它们的状态机，
    /// 并在服务器权威地结算撞击伤害。挂在玩家根物体上。
    /// </summary>
    /// <remarks>
    /// <b>索敌规则</b>：使魔<b>不会主动找怪</b>。它只通过 <see cref="IServerCombatResolutionListener"/>
    /// 监听「玩家攻击命中敌人」的结算记录，把被命中的敌人登记为候选目标（索敌范围 <c>AcquireRange</c> 内才算）。
    /// 使魔自身造成的伤害带 <c>DamageTags.Familiar</c> 标签，会被这里过滤掉，因此不会自触发。
    /// </remarks>
    /// <remarks>
    /// <b>战斗行为</b>：撞上目标后不停留，而是穿过目标 → 掉头 → 再冲，来回穿插，
    /// 直到目标死亡 / 玩家换目标 / 候选过期 / 达到单目标穿插上限。
    /// 伤害<b>无穿透次数上限</b>：路径上扫到的敌人一律结算，只受 <c>HitCooldownPerTarget</c> 节流。
    /// </remarks>
    /// <remarks>
    /// <b>网络</b>：状态机与伤害结算只在服务器推进（<c>NetworkObject.IsServer</c>），与圆型领域、喷火器一致。
    /// 本类只向 IImpactFamiliarPresentationPublisher 发布只读姿态，不直接创建或复制表现对象。
    /// </remarks>
    [DisallowMultipleComponent]
    public sealed partial class ImpactFamiliarController : MonoBehaviour, IServerCombatResolutionListener, IFamiliarPactTarget
    {
        private const float EnemyScanInterval = 0.2f;

        [Header("配置")]
        [Tooltip("使魔配置资产（asset）：所有可调参数都在这里。")]
        [SerializeField] private ImpactFamiliarAsset definitionAsset;
        [Tooltip("使魔数量：多只各自独立维护候选目标缓存，沿轨道均匀分布。")]
        [SerializeField, Min(1)] private int familiarCount = 2;
        [Tooltip("开局即激活：血契系统未接线时用于单机/调试验证。")]
        [SerializeField] private bool activeOnStart;

        [Header("统一环绕队列")]
        [Tooltip("队列归属者（owner）：同一个 owner 名下的所有使魔（含别的种类，如射击僚机）会排进同一条" +
        ...
        [Header("调试")]
        [Tooltip("在 Console 打印候选登记与起飞/返回，便于确认链路是否通。")]
        [SerializeField] private bool logToConsole;

        private readonly List<ImpactFamiliarBrain> m_Brains = new List<ImpactFamiliarBrain>();
        private readonly FamiliarTargetQuery m_TargetQuery = new FamiliarTargetQuery();

        private ImpactFamiliarDefinition m_Definition;
        ...
        private IImpactFamiliarPresentationPublisher m_PresentationPublisher;
        ...
        private void Awake()
        {
            ...
            for (int i = 0; i < behaviours.Length; i++)
            {
                if (!(behaviours[i] is IImpactFamiliarPresentationPublisher publisher)) continue;
        ...
        // 配置/血契接口见 ImpactFamiliarController.Configuration.cs。

        // 候选、状态推进、索敌与伤害见 ImpactFamiliarController.Runtime.cs。

        // 只读表现状态适配见 ImpactFamiliarController.Presentation.cs；实例化由 Presentation 层处理。
        ...
        private void ComposeIfNeeded()
        {
            ...
            m_Definition = definitionAsset.CreateDefinition();
        ...
        private void RebuildBrains()
        {
            ...
            {
                var brain = new ImpactFamiliarBrain(m_Definition, i, count, owner);
